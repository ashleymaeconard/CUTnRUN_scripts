...
# Runs Picard's MarkDuplicates in /gpfs/data/compbio/aconard/tools/picard.jar for all .sorted.bam files in a given directory

PICARD="/gpfs/data/compbio/aconard/tools/picard.jar"

# Check to make sure input is correct
if [ $# -ne 2 ]; then
	echo $0: Usage: "./run_picard_markduplicates.sh /PATH/TO/SORTED.BAMS/FOLDER(s)/ (containing folders of .sorted.bam files, assuming at most 2 subdirectories (e.g. only write up to bowtie2 here: .../results/bowtie2/clamp_sg_fm/MR_1-1/out.sorted.bam) PROCESSORS"
	exit 1
fi
    ...
    if [ -d ${dir} ] ; then
        ...
        for file in ${dir}/*/*sorted.bam
            do
                ...
                echo "(java -jar $PICARD MarkDuplicates I=$file O=$folderName/out.sorted_nodup.bam M=${folderName}/marked_dup_metrics.txt REMOVE_DUPLICATES=true) &" >> $COMMAND_SCRIPT
