...
# Runs MACs2 in /gpfs/main/home/aconard/venv/bin/macs2 for all .sorted.bam files in a given directory
# Resource: http://liulab.dfci.harvard.edu/MACS/FAQ.html

# Check to make sure input is correct
if [ $# -ne 2 ]; then
	echo $0: "Usage: ./run_macs2.sh /PATH/TO/SORTED.BAMS/FOLDER(s)/ (containing folders of .sorted.bam files, assuming at most 2 subdirectories (e.g. only write up to /bowtie2/ here: .../results/bowtie2/clamp_sg_fm/MR_1-1/out.sorted.bam ) PROCESSORS" #MFOLD_LOWER MFOLD_UPPER (default is 5 50) \n" # NOTE: MAKE SURE source ~/venv/bin/activate FOR PROPER PYTHON ENVIRONMENT.
	exit 1
fi
...
for dir in $INPUT_DIR/*/
    ...
    if [ -d ${dir} ] ; then
        ...
        for file in ${dir}/*/*.sorted.bam
            ...
            echo "(macs2 callpeak -t $file -B -f AUTO --nomodel --SPMR --keep-dup all --broad -g dm --trackline -n $fileName --cutoff-analysis --call-summits -p 0.05 --outdir $folderName 2> $folderName/$fileName.log) &" >> $COMMAND_SCRIPT  # -m ${MFOLD_LOWER} ${MFOLD_UPPER}
        done
    fi
done
